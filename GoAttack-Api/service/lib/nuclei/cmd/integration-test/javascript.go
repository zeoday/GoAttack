...
func purge(resource *dockertest.Resource) {
	if resource != nil && pool != nil {
		...
		if err != nil {
			log.Info("Could not purge resource: %s", err)
		}
...
func init() {
	...
	if err != nil {
		log.Info("something went wrong with dockertest: %s", err)
		return
	}
	...
	if err != nil {
		log.Info("Could not connect to Docker: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start resource: %s", err)
		return
	}
	// by default expire after 30 sec
	if err := redisResource.Expire(30); err != nil {
		log.Info("Could not expire resource: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start Oracle resource: %s", err)
		return
	}

	// by default expire after 30 sec
	if err := oracleResource.Expire(30); err != nil {
		log.Info("Could not expire Oracle resource: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start postgres resource: %s", err)
		return
	}
	// by default expire after 30 sec
	if err := postgresResource.Expire(30); err != nil {
		log.Info("Could not expire postgres resource: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start mysql resource: %s", err)
		return
	}
	// by default expire after 30 sec
	if err := mysqlResource.Expire(30); err != nil {
		log.Info("Could not expire mysql resource: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start Rsync resource: %s", err)
		return
	}
	// by default expire after 30 sec
	if err := rsyncResource.Expire(30); err != nil {
		log.Info("Could not expire Rsync resource: %s", err)
	}
	...
	if err != nil {
		log.Info("Could not start Telnet resource: %s", err)
		return
	}
	// by default expire after 30 sec
	if err := telnetResource.Expire(30); err != nil {
		log.Info("Could not expire Telnet resource: %s", err)
	}
}
